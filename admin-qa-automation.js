...
class EnhancedApplyNoteAutomation {
    constructor(options = {}) {
        ...
        this.selectors = {
            // Login selectors (multiple fallbacks)
            login: {
                email: [
                    'input[type="email"]',
                    'input[name="email"]',
    ...
    // Enhanced element finder with multiple selector strategies
    ...
    // Enhanced action executor with better selector support
    async executeAction(action, selector, value = '') {
        ...
        try {
            switch(action.toLowerCase().trim()) {
                ...
                case 'doubleclick':
                    await this.page.dblclick(selector, { timeout: 10000 });
                    break;
                    
                case 'rightclick':
                    await this.page.click(selector, { button: 'right', timeout: 10000 });
                    break;
    ...
    parseTestSteps(testSteps) {
        ...
        for (const line of lines) {
            ...
            if (trimmedLine.toLowerCase().includes('click')) {
                step.action = 'click';
                // Extract element to click
                const clickMatch = trimmedLine.match(/click\s+(.+?)(?:\s|$)/i);
                if (clickMatch) {
                    step.selector = this.mapElementToSelector(clickMatch[1]);
                }
            } else if (trimmedLine.toLowerCase().includes('enter') || trimmedLine.toLowerCase().includes('type')) {
                step.action = 'type';
                // Extract text and element
                const typeMatch = trimmedLine.match(/(?:enter|type)\s+(.+?)(?:\s+in|\s|$)/i);
                if (typeMatch) {
                    step.value = typeMatch[1];
                    step.selector = this.mapElementToSelector('input');
                }
            } else if (trimmedLine.toLowerCase().includes('navigate')) {
                step.action = 'navigate';
                const urlMatch = trimmedLine.match(/navigate\s+(.+?)(?:\s|$)/i);
                if (urlMatch) {
                ...
                step.action = 'wait';
                step.value = '2000';
            } else if (trimmedLine.toLowerCase().includes('verify')) {
                step.action = 'verifytext';
                const verifyMatch = trimmedLine.match(/verify\s+(.+?)(?:\s+is|\s|$)/i);
                if (verifyMatch) {
                    step.selector = 'body';
                    step.value = verifyMatch[1];
                }
            }
    ...
    // Map element descriptions to selectors
    mapElementToSelector(element) {
        const elementMap = {
            'login button': this.selectors.login.submit.join(', '),
            'email': this.selectors.login.email.join(', '),
            'password': this.selectors.login.password.join(', '),
            'dashboard': this.selectors.navigation.dashboard.join(', '),
            'users': this.selectors.navigation.users.join(', '),
            'settings': this.selectors.navigation.settings.join(', '),
            'input': this.selectors.forms.input.join(', '),
            'button': this.selectors.forms.button.join(', '),
        ...
        return element; // Return as-is if no mapping found
    }

    // Run functional tests
    async runFunctionalTests(excelPath, sheetName = 'Admin') {
        const excelReader = new CustomExcelReader(excelPath);
        const testCases = excelReader.getTestCases(sheetName);
        
        console.log(chalk.cyan(`\nFound ${testCases.length} test cases in ${sheetName}`));
        ...
        for (const testCase of testCases) {
            ...
            try {
                if (sheetName === 'Admin' && testCase.test_steps) {
                    ...
                    // For UI compliance tests, take comprehensive screenshots
    ...
    generateReport() {
        ...
        console.log(chalk.cyan('\n\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550'));
        console.log(chalk.cyan('           FUNCTIONAL TEST SUMMARY        '));
        console.log(chalk.cyan('\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550'));
        ...
        const reportPath = path.join(this.reportDir, `functional-test-report-${format(new Date(), 'yyyyMMdd-HHmmss')}.json`);
        fs.writeFileSync(reportPath, JSON.stringify(this.testResults, null, 2));
        console.log(chalk.blue(`\nFull report saved to: ${reportPath}`));
    }
}

module.exports = EnhancedApplyNoteAutomation;
